...
The Scala backend exposes a `PideBridge` trait (with a default `LocalSyntaxPideBridge` that preserves today's command-span-and-disclaimer behavior) so milestones 4, 5, and 7's PIDE-backed document status, entity metadata, structured proof state, and Sledgehammer proof search plug into a clear interface without changing the JSON-RPC protocol or the VS Code extension. A second experimental bridge, `IsabelleCliPideBridge`, has been scaffolded for cross-platform Isabelle CLI access (Linux, macOS, and Windows via Isabelle's bundled Cygwin layer); it currently performs only a bounded `isabelle version` reachability probe and otherwise delegates to the local-syntax fallback, with follow-up PRs to wire real CLI/ML-driven document and proof-state queries.
