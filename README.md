...
- PIDE-mode configuration plumbing: `isabelle.pide.mode` (`localSyntax` default, `scalaIsabelle` opt-in) plus `isabelle.pide.isabelleHome`, `isabelle.pide.userDir`, `isabelle.pide.sessionName`, and `isabelle.pide.logicSession` settings, an `Isabelle: Configure PIDE Mode` command, and a typed `pide/configure` JSON-RPC method that the Scala backend stores in an `AtomicReference`. The default behavior is unchanged: the backend keeps using `LocalSyntaxPideBridge` and always reports `activeBridge: "local-syntax"`. Live wiring of the scala-isabelle bridge and any actual PIDE calls remain a follow-up PR.
- Unit tests for protocol framing, request correlation, ROOT parsing, workspace session discovery, theory graph construction, build command generation, diagnostic parsing, semantic tokenization, repair request capture, patch preview safety, command-span extraction, document status summaries, proof-outline helpers, and PIDE-mode configuration parameter building.
