...
- **Structural code folding** — fold `proof … qed` blocks, `begin … end` bodies (locale / class / `instantiation` / `context` / `notepad`), `text ‹…›` / `ML ‹…›` cartouche blocks, the `section`/`subsection` hierarchy, multi-line comments, and the `theory … begin` header (comment/cartouche/string aware).
- **Smart selection** — <kbd>⌥/Alt</kbd>+<kbd>Shift</kbd>+<kbd>→</kbd> expands the selection through the surrounding structure: token → quoted term/cartouche → command → enclosing proof or `begin … end` block → whole theory.
...
- **Find All References** — <kbd>Shift</kbd>+<kbd>F12</kbd> lists every use of the name under the cursor across the workspace's `.thy` files (a name-based, offline search; the prover is the authority on scope).
