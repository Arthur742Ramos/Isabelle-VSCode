...
**Phase 3c solution (this PR)**: per-command FILTERING is now implemented at the reflective extractor's tuple-walk layer (NOT via `snapshot.cumulate`, which would have required additional reflective surface for `Document.Snapshot.cumulate` overloads). The extractor probes each `snapshot.messages` entry's first element for a zero-arg `range()` accessor (matching `Text.Info[_].range`) returning an object with `start`/`stop` Int accessors (matching `isabelle.Text.Range`). The collected `(Option[(Int, Int)], String)` list is then filtered against the cursor's command range via a `MessageFilterMode` enum:

- `CursorCommandOnly` (default for `proofState/getWithPide`): keep positioned entries whose range overlaps the cursor's command. If no entry has positional info, fall back to including everything (with a structured note in the response). Unpositioned entries in a mixed-position result are dropped — otherwise Phase 3c would re-introduce the whole-file noise it's meant to remove.
- `WholeSnapshot` (explicit opt-in from `SledgehammerWithPideHandler`): keep every entry regardless of position. Required because Sledgehammer "Try this:" output may be unpositioned OR positioned on a range outside the injected `sledgehammer` command itself.

The half-open overlap predicate `rangesOverlap` is package-private and pinned by tests in `SnapshotProofStateExtractorSpec` covering: identical ranges, strict overlap, containment, edge-touching (no overlap), disjoint, zero-length (no overlap). The filter policy is similarly pinned with synthetic-input tests so future refactors cannot silently regress the per-cursor-focus behavior OR silently regress Sledgehammer's whole-file harvest.
