...
export function buildRepairRequestMarkdown(snapshot: RepairRequestSnapshot): string {
  return [
    ...
    "## Proposed repair",
    "",
    "Paste or save a proposed unified diff separately, then run `Isabelle: Preview Repair Patch`. The preview command is read-only and rejects unsafe patch shapes.",
    "",
    "After manually applying trusted edits, run `Isabelle: Check Current Workspace for Repair` to validate the current workspace contents with the existing Isabelle build command.",
    ""
  ].join("\n");
}
