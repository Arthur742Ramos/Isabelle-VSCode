...
This checklist exercises every LSP-mode capability the extension advertises against `examples/Smoke.thy`. It exists because the structural test suite (749 cases as of 2026-05-18) intentionally avoids importing `vscode` and cannot validate the live PIDE round-trip. See [`docs/ROADMAP_STATUS.md`](ROADMAP_STATUS.md) "Tier-2 manual verifications" for the master list this file expands on.
...
6. **Build:** run `Isabelle: Build Active Session`. Confirm the output channel scrolls and the build succeeds.
...
- [ ] **Minimization.** *Upstream-blocked* — there is no LSP surface for this today. Skip.

### M8 — Theory graph + proof-engineering

- [ ] Open the **Isabelle Theory Graph** view. Confirm `Smoke` appears under its session with `Main` as a dependency.
