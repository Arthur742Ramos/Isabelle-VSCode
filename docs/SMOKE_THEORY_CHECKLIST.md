...
This checklist exercises every LSP-mode capability the extension advertises against `examples/Smoke.thy`. It exists because the structural test suite (see `npm run test` / CI for the current count — per AGENTS.md gotcha #7 we don't pin a number here) intentionally avoids importing `vscode` and cannot validate the live PIDE round-trip. See [`docs/ROADMAP_STATUS.md`](ROADMAP_STATUS.md) "Tier-2 manual verifications" for the master list this file expands on.
